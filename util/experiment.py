"""Utility functions for data generation in experiments."""
...
def generate_data_1d(N_train=20, N_test=20,
                     N_calib=500, N_calib_induce=500,
                     N_valid=500,
                     noise_sd=0.03,
    ...
    # calib_sample_id = np.argmin(np.abs(X_valid - X_test[None, :]), axis=0)

    calib_sample_id = np.linspace(0, N_valid - 1,
                                  num=N_calib).astype(np.int)

    calib_sample_id_induce = np.linspace(0, N_valid - 1,
                                         num=N_calib_induce).astype(np.int)
    # organize shapes for train/test data
    ...
    return (X_train, y_train, X_test, y_test,
            X_valid, y_valid_sample,
            calib_sample_id, calib_sample_id_induce)
